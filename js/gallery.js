...
window.gallery = (function () {
  ...
  window.util.initPopup(editingModal, window.editingForm.onEscPress);
  var editingForm = document.querySelector('.img-upload__form');
  var fileField = editingForm.querySelector('#upload-file');
  var previewImg = editingForm.querySelector('.img-upload__preview img');
  var previewMiniatures = editingForm.querySelectorAll('.effects__preview');

  fileField.addEventListener('change', function () {
    window.util.loadGraficFile(fileField.files[0], previewImg, previewMiniatures, window.util.openPopup.bind(null, editingModal, window.editingForm.onEscPress));
  });
  ...
  var effectRange = editingForm.querySelector('.img-upload__effect-level');
  ...
  window.editingForm.resetCustomControls();
  ...
  editingForm.addEventListener('submit', function (evt) {
    evt.preventDefault();
    window.backend.upload(new FormData(editingForm), onUploadSuccess, onUploadError);
  });
